    ...
    for (let t in targets) {
        for (let a in targets[t][attribute]) {
            ...
            if (typeof hash !== 'undefined') {
                if (asset[hash]) {
                    hashList.push(asset[hash]);
                } else if (hash === 'md5ext') {
                    // Some projects don't have an md5ext field for some reason,
                    // extract that data from the assetId and dataFormat
                    hashList.push(asset.assetId + '.' + asset.dataFormat);
                }
            }
        }
    }
...
const metadata = function (meta) {
    let obj = {};
    for (const key of ['semver', 'vm', 'agent', 'origin']) {
